    ...
    : QMainWindow(parent)
{
    ...
    label->setText(tr("Добавить в текстовый редактор возможность вставлять дату и время в текст по позиции курсора."));
    ...
    pasteDateTime = menuBar()->addMenu(tr("Вставить"));

    pasteDateAct = pasteDateTime->addAction(tr("Дата"));
    connect(pasteDateAct, SIGNAL(triggered()), this, SLOT(pasteDate()));

    pasteTimeAct = pasteDateTime->addAction(tr("Время"));
    connect(pasteTimeAct, SIGNAL(triggered()), this, SLOT(pasteTime()));
...
void MainWindow::changeEvent(QEvent *event)
{
    ...
    if (event->type() == QEvent::LanguageChange) {
        this->label->setText(tr("Добавить в текстовый редактор возможность вставлять дату и время в текст по позиции курсора."));
        setWindowTitle(tr("Текстовый редактор"));
        tEdit->setWindowTitle(tr("Безымянный"));
        ...
        pasteDateTime->setTitle(tr("Вставить"));
        pasteDateAct->setText(tr("Дата"));
        pasteTimeAct->setText(tr("Время"));
...
void MainWindow::pasteTime()
{
    ...
    QString str = tr(" Время: ") + now.time().toString() + " ";
    QMdiSubWindow *activeSubWindow = mdiArea->activeSubWindow();
    QWidget *wgt = activeSubWindow->widget();
    tEdit = (QTextEdit*)wgt;
...
{
    QDateTime now = QDateTime::currentDateTime();
    QString str = tr(" Дата: ") + now.date().toString("dd.MM.yyyy") + " ";
    QMdiSubWindow *activeSubWindow = mdiArea->activeSubWindow();
    QWidget *wgt = activeSubWindow->widget();
    tEdit = (QTextEdit*)wgt;
    QTextCursor cursor(tEdit->textCursor());
